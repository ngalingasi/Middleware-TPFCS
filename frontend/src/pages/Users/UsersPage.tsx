...
function UserForm({ user, allSkills, isSuperAdmin: superAdmin, onSaved, onClose }: {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userId) await usersApi.updateSkills(userId, selectedSkills);
...
function ResetPasswordForm({ user, onClose }: { user: UserRecord; onClose: () => void }) {
  ...
  const handleReset = async () => {
    setSaving(true);
    try { await usersApi.update(user.user_id, { must_change_password: 1 } as any); setDone(true); }
    finally { setSaving(false); }
  };
  if (done) return (
...
export default function UsersPage() {
  ...
  useEffect(() => { load(); }, [load]);
  useEffect(() => { usersApi.getSkills().then(r => setAllSkills(r.data as any)).catch(() => {}); }, []);

  const handleDelete = async () => {
    if (!selected) return;
    setDeleting(true);
    try { await usersApi.delete(selected.user_id); toast.success('User deactivated'); await load(); setModal(null); }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                ...
                {isSuperAdmin && <th className="px-4 py-3 text-left font-medium text-gray-600 dark:text-gray-400">ICDV</th>}
                ...
                Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    {Array.from({ length: isSuperAdmin ? 5 : 4 }).map((_, j) => (
                      <td key={j} className="px-4 py-3"><div className="h-4 bg-gray-100 dark:bg-gray-800 rounded w-20" /></td>
                    ))}
                  </tr>
                ))
              ) : !Array.isArray(data?.results) || data.results.length === 0 ? (
                <tr><td colSpan={isSuperAdmin ? 6 : 5} className="px-4 py-12 text-center text-gray-400">No users found</td></tr>
              ) : data.results.map(u => (
                <tr key={u.user_id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                  ...
                  {isSuperAdmin && (
                    <td className="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                      {u.icdv_name ?? <span className="italic text-gray-300">—</span>}
                    </td>
                  )}
                  <td className="px-4 py-3">
        ...
        {data && data.totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              <button onClick={() => setPage(p => p - 1)} disabled={page === 1} className="px-3 py-1.5 text-sm rounded-lg border border-gray-300 dark:border-gray-700 disabled:opacity-40">Previous</button>
