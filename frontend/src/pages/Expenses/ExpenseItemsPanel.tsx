...
export default function ExpenseItemsPanel() {
  const [items,   setItems]   = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<any | null>(null); // null = closed, {} = new, object = edit
  const [form,    setForm]    = useState({ name: '', description: '', default_rate: '', unit: 'unit', status: 'active' });
  const [saving,  setSaving]  = useState(false);
  const [error,   setError]   = useState<string|null>(null);

  const load = () => {
    setLoading(true);
    expensesApi.listItems().then(r => setItems(r.data)).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, []);

  const openNew  = () => { setForm({ name: '', description: '', default_rate: '', unit: 'unit', status: 'active' }); setEditing({}); setError(null); };
  ...
  const save = async () => {
    ...
    try {
      if (editing?.item_id) await expensesApi.updateItem(editing.item_id, { ...form, default_rate: parseFloat(form.default_rate) || 0 });
      else                  await expensesApi.createItem({ ...form, default_rate: parseFloat(form.default_rate) || 0 });
      close(); load();
  ...
  const deactivate = async (id: number) => {
    if (!window.confirm('Deactivate this item? It will no longer appear in new expenses.')) return;
    await expensesApi.deleteItem(id).catch(() => {});
    load();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {editing !== null && (
        <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 space-y-4 relative z-[100001]">
            <h2 className="text-base font-bold text-gray-800 dark:text-white">{editing?.item_id ? 'Edit Item' : 'New Expense Item'}</h2>
      ...
      {loading
        ...
        : items.length === 0
          ? <p className="text-sm text-gray-400">No catalog items yet. Create one to use in expenses.</p>
          : (
